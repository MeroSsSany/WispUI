...
import dev.merosssany.wispui.buffer.NFloatBuffer;
import dev.merosssany.wispui.data.RGBA;
import dev.merosssany.wispui.data.ResourceLoader;
import dev.merosssany.wispui.data.template.window.DefaultWindowParameterHint;
import dev.merosssany.wispui.data.template.window.WindowParameterHint;
...
public class Window {
    ...
    protected final Logger logger = LoggerFactory.getLogger(Window.class);
    
    protected final int[] tempW = new int[1], tempH = new int[1];
    protected final double[] xpos = new double[1], ypos = new double[1];
     ...
     */
    protected void initGLFW(WindowParameterHint parameter) {
    ...
    public Vector2f getContentScale() {
        NFloatBuffer x = new NFloatBuffer(1);
        NFloatBuffer y = new NFloatBuffer(1);
        
        glfwGetWindowContentScale(window, x.getBuffer(), y.getBuffer());
        
        Vector2f scale = new Vector2f(x.get(), y.get());
        x.cleanup();
        y.cleanup();

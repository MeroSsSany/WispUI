package dev.merosssany.wispui.manager;
...
import static dev.merosssany.wispui.VectorMath.isPointWithinRectangle;

/**
 * The central controller for UI navigation, rendering order, and input routing.
 * <p>
 * SceneManager maintains a global registry of available scenes and a dynamic stack
 * of active scenes. It ensures that inputs (clicks/hovers) are processed by the
 * top-most visual layer first, preventing "click-through" to background screens.
 * </p>
 *
 *
 *
 * <h2>Core Systems</h2>
 * <ul>
 * <li><b>The Scene Stack:</b> Handles transitions. {@code pushScreen} adds an overlay,
 * while {@code setScreen} clears the stack for a fresh state (e.g., going from Main Menu to Game).</li>
 * <li><b>Recursive Drawing:</b> Renders from the bottom of the stack up, allowing
 * transparent overlays to see the content beneath them.</li>
 * <li><b>Top-Down Input:</b> Propagates mouse events from the top (front) of the UI list
 * to the bottom (back), stopping once an element consumes the event.</li>
 * </ul>
 */
    ...
    public static void propagateMouseClick(MouseButtonEvent e, List<UI> uis, int offestX, int offestY) {
        ...
        for (int i = uis.size() -1; i >= 0; i--) {
            ...
            if (isPointWithinRectangle(x, y, mousePosition.x, mousePosition.y, width, height)) {
                ui.onMouseClicked(e, mousePosition.x - x, y - mousePosition.y);
